...
public class Player : MonoBehaviour
{
    ...
    public float ASkillCoolDown;
    public float SSkillCoolDown;
    public float DSkillCoolDown;
    ...
    bool ASkillCoolDownReady;
    bool SSkillCoolDownReady;
    bool DSkillCoolDownReady;
    bool FSkillCoolDownReady;
    ...
    void PlayerUseSkill()
    {
        ...
        ESkillCoolDownReady = SkillCoolTime[2] < ESkillCoolDown;

        RSkillCoolDown += Time.deltaTime;
        RSkillCoolDownReady = SkillCoolTime[3] < RSkillCoolDown;

        ASkillCoolDown += Time.deltaTime;
        ASkillCoolDownReady = SkillCoolTime[4] < ASkillCoolDown;

        SSkillCoolDown += Time.deltaTime;
        SSkillCoolDownReady = SkillCoolTime[5] < SSkillCoolDown;

        DSkillCoolDown += Time.deltaTime;
        DSkillCoolDownReady = SkillCoolTime[6] < DSkillCoolDown;

        FSkillCoolDown += Time.deltaTime;
        FSkillCoolDownReady = SkillCoolTime[7] < FSkillCoolDown;
        ...
        if (EDown && !EnemySumRedy && !IsUseSkill && ESkillCoolDownReady && !ActionController)
        {
            StartCoroutine(UseEskill(Mpos));
        }

        if (RDown && !EnemySumRedy && !IsUseSkill && RSkillCoolDownReady && !ActionController)
        {
            StartCoroutine(UseRskill(Mpos));
        }

        if (ADown && !EnemySumRedy && !IsUseSkill && ASkillCoolDownReady && !ActionController)
        {
            StartCoroutine(UseAskill(Mpos));
        }

        if (SDown && !EnemySumRedy && !IsUseSkill && SSkillCoolDownReady && !ActionController)
        {
            StartCoroutine(UseSskill(Mpos));
        }

        if (DDown && !EnemySumRedy && !IsUseSkill && DSkillCoolDownReady && !ActionController)
        {
            StartCoroutine(UseDskill(Mpos));
        }

        if (FDown && !EnemySumRedy && !IsUseSkill && FSkillCoolDownReady && !ActionController)
        {
            StartCoroutine(UseFskill(Mpos));
        }
    }
    ...
    IEnumerator UseEskill(Vector3 Mpos)
    {
        if (_MainSkill[2] != null)
        {
            isMove = false;
            anim.SetBool("IsWalk", false);
            IsUseSkill = true;
            ...
            var newEskill = Instantiate(_MainSkill[2].SkillPrefab, Mpos, MissilePos.rotation);
            //Destroy(newEskill);
            WSkillCoolDown = 0;
            yield return null;
        }
    ...
    IEnumerator UseRskill(Vector3 Mpos)
    {
        if (_MainSkill[3] != null)
        {
            isMove = false;
            anim.SetBool("IsWalk", false);
            IsUseSkill = true;
            ...
            var newRskill = Instantiate(_MainSkill[3].SkillPrefab, Mpos, MissilePos.rotation);
            //Destroy(newRskill);
            WSkillCoolDown = 0;
            yield return null;
        }
    ...
    IEnumerator UseAskill(Vector3 Mpos)
    {
        if (_MainSkill[4] != null)
        {
            isMove = false;
            anim.SetBool("IsWalk", false);
            IsUseSkill = true;
            ...
            var newAskill = Instantiate(_MainSkill[4].SkillPrefab, Mpos, MissilePos.rotation);
            //Destroy(newAskill);
            WSkillCoolDown = 0;
            yield return null;
        }
    ...
    IEnumerator UseSskill(Vector3 Mpos)
    {
        if (_MainSkill[5] != null)
        {
            isMove = false;
            anim.SetBool("IsWalk", false);
            IsUseSkill = true;
            ...
            var newSskill = Instantiate(_MainSkill[5].SkillPrefab, Mpos, MissilePos.rotation);
            //Destroy(newSskill);
            WSkillCoolDown = 0;
            yield return null;
        }
    ...
    IEnumerator UseDskill(Vector3 Mpos)
    {
        if (_MainSkill[6] != null)
        {
            isMove = false;
            anim.SetBool("IsWalk", false);
            IsUseSkill = true;
            ...
            var newDskill = Instantiate(_MainSkill[6].SkillPrefab, Mpos, MissilePos.rotation);
            //Destroy(newDskill);
            WSkillCoolDown = 0;
            yield return null;
        }
    ...
    IEnumerator UseFskill(Vector3 Mpos)
    {
        if (_MainSkill[7] != null)
        {
            isMove = false;
            anim.SetBool("IsWalk", false);
            IsUseSkill = true;
            ...
            var newFskill = Instantiate(_MainSkill[7].SkillPrefab, Mpos, MissilePos.rotation);
            //Destroy(newFskill);
            WSkillCoolDown = 0;
            yield return null;
        }
