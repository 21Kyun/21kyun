...
public class Enemy : MonoBehaviour
{
    ...
    {
        if (other.tag == "DamageField")
        {
            if (!isDamage)
            {
                WPMobj Damagefield = other.GetComponent<WPMobj>();
                WPM WPM_Script = (WPM)FindObjectOfType(typeof(WPM));
                curHealth -= WPM_Script.damage;
                Vector3 reactVec = new Vector3(0, 0, 0);
                StartCoroutine(OnDamage(reactVec));

                Debug.Log("WPMDam :" + curHealth);
                //Debug.Log(mat.color);
            }
